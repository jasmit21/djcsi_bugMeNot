...
import com.example.yourlinkapp.activities.ParentSignedInActivity;
import com.example.yourlinkapp.utils.Constant;

import static com.example.yourlinkapp.NotificationChannelCreator.CHANNEL_ID;

public class GeoFencingForegroundService extends android.app.Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent.getAction() != null) {
			if (intent.getAction().equals(Constant.ACTION_STOP_GEO_FENCING_SERVICE)) {
				closeFencingService();
				android.util.Log.i(TAG, "onStartCommand: lastEmail: " + lastChildEmail);
			}
		}
		...
		android.util.Log.i(TAG, "onStartCommand: service started");
			...
			public void onDataChange(@androidx.annotation.NonNull DataSnapshot dataSnapshot) {
				if (dataSnapshot.exists()) {
					...
					geoFencingQuery.addValueEventListener(new ValueEventListener() {
						@Override
						public void onDataChange(@androidx.annotation.NonNull DataSnapshot dataSnapshot) {
							android.util.Log.i(TAG, "onDataChange: value: " + dataSnapshot.getValue());
	...
	private void showNotification(DataSnapshot dataSnapshot, String childName) {
		android.util.Log.i(TAG, "showNotification: key: " + dataSnapshot.getKey());
		android.util.Log.i(TAG, "showNotification: value: " + dataSnapshot.getValue());
			...
			query.addListenerForSingleValueEvent(new ValueEventListener() {
				...
				public void onDataChange(@androidx.annotation.NonNull DataSnapshot dataSnapshot) {
					if (dataSnapshot.exists()) {
						final DataSnapshot nodeShot = dataSnapshot.getChildren().iterator().next();
						String childUID = nodeShot.getKey();
						android.util.Log.i(TAG, "onDataChange: Done");
