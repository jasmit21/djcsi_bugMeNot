...
public class ScreenTimeReceiver extends BroadcastReceiver {
	...
	public ScreenTimeReceiver(ScreenLock screenLock) {
		...
		android.util.Log.i(TAG, "ScreenTimeReceiver: allowedTime: " + allowedTime);
	}
	...
	public void onReceive(Context context, Intent intent) {
		if (intent.getAction().equals(Intent.ACTION_SCREEN_ON)) {
			...
			android.util.Log.i(TAG, "onReceive: endTime: " + endTime);
			android.util.Log.i(TAG, "onReceive: period: " + period);
			
			if (period != startTime && period != endTime && period != 0) {
				screenTime += period / 1000;    //seconds
				android.util.Log.i(TAG, "onReceive: screenTime: " + screenTime + "s");
			}
		}
