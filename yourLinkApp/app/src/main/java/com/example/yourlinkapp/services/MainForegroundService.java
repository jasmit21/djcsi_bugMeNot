package com.example.yourlinkapp.services;
...
import com.example.yourlinkapp.activities.BlockedAppActivity;
import com.example.yourlinkapp.activities.ChildSignedInActivity;
import com.example.yourlinkapp.broadcasts.AppInstalledReceiver;
import com.example.yourlinkapp.broadcasts.AppRemovedReceiver;
import com.example.yourlinkapp.broadcasts.PhoneStateReceiver;
import com.example.yourlinkapp.broadcasts.ScreenTimeReceiver;
import com.example.yourlinkapp.broadcasts.SmsReceiver;
...
import com.example.yourlinkapp.models.ScreenLock;
...
import static com.example.yourlinkapp.NotificationChannelCreator.CHANNEL_ID;

public class MainForegroundService extends android.app.Service {
	public static final int NOTIFICATION_ID = 27;
	public static final String TAG = "MainServiceTAG";
	public static final String BLOCKED_APP_NAME_EXTRA = "com.example.yourlinkapp.services.BLOCKED_APP_NAME_EXTRA";
	...
	private FirebaseDatabase firebaseDatabase = FirebaseDatabase.getInstance("https://yourlink-19e96-default-rtdb.firebaseio.com/");
	...
	public void onCreate() {
		...
		android.util.Log.i(TAG, "onCreate: executed");
	}
		...
		screenTimeQuery.addValueEventListener(new ValueEventListener() {
			...
			public void onDataChange(@androidx.annotation.NonNull DataSnapshot dataSnapshot) {
				if (dataSnapshot.exists()) {
					...
					android.util.Log.i(TAG, "onDataChangeX: hours: " + screenLock.getHours());
					android.util.Log.i(TAG, "onDataChangeX: minutes: " + screenLock.getMinutes());
					android.util.Log.i(TAG, "onDataChangeX: isLocked: " + screenLock.isLocked());
		...
		query.addListenerForSingleValueEvent(new ValueEventListener() {
			...
			public void onDataChange(@androidx.annotation.NonNull DataSnapshot dataSnapshot) {
				if (dataSnapshot.exists()) {
					...
					android.util.Log.i(TAG, "onDataChange: child name: " + child.getName());
					//updateAppStats(apps);
	...
	private void getUserLocation() {
		android.util.Log.i(TAG, "getUserLocation: executed");
		...
		LocationListener locationListener = new LocationListener() {
			...
			public void onLocationChanged(Location location) {
				if (location != null) {
					android.util.Log.i(TAG, "onLocationChanged: latitude: " + location.getLatitude());
					android.util.Log.i(TAG, "onLocationChanged: longitude: " + location.getLongitude());
					addUserLocationToDatabase(location, uid);
				} else {
					android.util.Log.i(TAG, "onLocationChanged: location is null");
				}
			}
	...
	private void setFence(DataSnapshot dataSnapshot) {
		final com.example.yourlinkapp.models.Location childLocation = dataSnapshot.getValue(com.example.yourlinkapp.models.Location.class);
		android.util.Log.i(TAG, "setFence: getLatitude " + childLocation.getLatitude());
		android.util.Log.i(TAG, "setFence: getLongitude " + childLocation.getLongitude());
		android.util.Log.i(TAG, "setFence: isGeoFence " + childLocation.isGeoFence());
		android.util.Log.i(TAG, "setFence: isOutOfFence " + childLocation.isOutOfFence());
		android.util.Log.i(TAG, "setFence: getFenceCenterLatitude " + childLocation.getFenceCenterLatitude());
		android.util.Log.i(TAG, "setFence: getFenceCenterLongitude " + childLocation.getFenceCenterLongitude());
		android.util.Log.i(TAG, "setFence: getFenceDiameter " + childLocation.getFenceDiameter());
		
		if (childLocation.isGeoFence()) {
			android.util.Log.i(TAG, "setFence: true");
			LocationManager locationManager = (LocationManager) getSystemService(LOCATION_SERVICE);
			LocationListener locationListener = new LocationListener() {
				@Override
				public void onLocationChanged(Location location) {
					android.util.Log.i(TAG, "setFence: changed");
					if (location != null) {
						...
						if (outOfFence) {
							android.util.Log.i(TAG, "setFence: OUT OF FENCE");
						...
						android.util.Log.i(TAG, "setFence: location is null");
					}
				}
	...
	public ArrayList<Contact> getContacts() {
		...
		android.database.Cursor cursor = contentResolver.query(ContactsContract.Contacts.CONTENT_URI, null, null, null, null);
		if (cursor.getCount() > 0) {
			while (cursor.moveToNext()) {
				String id = cursor.getString(cursor.getColumnIndex(ContactsContract.Contacts._ID));
				if (cursor.getInt(cursor.getColumnIndex(ContactsContract.Contacts.HAS_PHONE_NUMBER)) > 0) {
					android.database.Cursor cursorInfo = contentResolver.query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI, null, ContactsContract.CommonDataKinds.Phone.CONTACT_ID + " = ?", new String[]{id}, null);
	...
	private void getInstalledApplications(/*ArrayList<App> onlineAppsList*/) {
		...
		java.util.List<ApplicationInfo> applicationInfoList = packageManager.getInstalledApplications(0);
	...
	private void prepareData(java.util.List<ApplicationInfo> applicationInfoList, PackageManager packageManager/*, ArrayList<App> onlineAppsList*/) {
	...
	private void uploadApps(ArrayList<App> appsList) {
		...
		android.util.Log.i(TAG, "uploadApps: done");
	}
	...
	private String getLollipopForegroundAppPackageName() {
		...
		try {
			...
			Date date = new Date();
			java.util.List<UsageStats> foregroundApps = usageStatsManager.queryUsageStats(UsageStatsManager.INTERVAL_DAILY, date.getTime() - milliSecs, date.getTime());
			if (foregroundApps.size() == 0) {
				android.util.Log.i(TAG, "getLollipopForegroundAppPackageName: queryUsageSize: empty");
			}
			...
			for (UsageStats stats : foregroundApps) {
                /*if (i == 0 && !"com.example.yourlinkapp".equals(stats.getPackageName())) {
	...
	private String getKitkatForegroundAppPackageName() {
		ActivityManager activityManager = (ActivityManager) getSystemService(Context.ACTIVITY_SERVICE);
		java.util.List<ActivityManager.RunningAppProcessInfo> tasks = activityManager.getRunningAppProcesses();
		...
		public void run() {
			while (true) {
				...
				if (apps != null) {
					...
					android.util.Log.i(TAG, "run: foreground app: " + foregroundAppPackageName);
